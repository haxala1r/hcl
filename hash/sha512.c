#include "../include/hash.h"
#include "../include/common.h"
#include <stdlib.h>
#include <string.h>

/* This can be computed in run-time, but I decided it would be better to simply
 * hard-code them.
 */
uint64_t round_constants512[80] = {
	0x428a2f98d728ae22, 0x7137449123ef65cd, 0xb5c0fbcfec4d3b2f, 0xe9b5dba58189dbbc, 0x3956c25bf348b538,
	0x59f111f1b605d019, 0x923f82a4af194f9b, 0xab1c5ed5da6d8118, 0xd807aa98a3030242, 0x12835b0145706fbe,
	0x243185be4ee4b28c, 0x550c7dc3d5ffb4e2, 0x72be5d74f27b896f, 0x80deb1fe3b1696b1, 0x9bdc06a725c71235,
	0xc19bf174cf692694, 0xe49b69c19ef14ad2, 0xefbe4786384f25e3, 0x0fc19dc68b8cd5b5, 0x240ca1cc77ac9c65,
	0x2de92c6f592b0275, 0x4a7484aa6ea6e483, 0x5cb0a9dcbd41fbd4, 0x76f988da831153b5, 0x983e5152ee66dfab,
	0xa831c66d2db43210, 0xb00327c898fb213f, 0xbf597fc7beef0ee4, 0xc6e00bf33da88fc2, 0xd5a79147930aa725,
	0x06ca6351e003826f, 0x142929670a0e6e70, 0x27b70a8546d22ffc, 0x2e1b21385c26c926, 0x4d2c6dfc5ac42aed,
	0x53380d139d95b3df, 0x650a73548baf63de, 0x766a0abb3c77b2a8, 0x81c2c92e47edaee6, 0x92722c851482353b,
	0xa2bfe8a14cf10364, 0xa81a664bbc423001, 0xc24b8b70d0f89791, 0xc76c51a30654be30, 0xd192e819d6ef5218,
	0xd69906245565a910, 0xf40e35855771202a, 0x106aa07032bbd1b8, 0x19a4c116b8d2d0c8, 0x1e376c085141ab53,
	0x2748774cdf8eeb99, 0x34b0bcb5e19b48a8, 0x391c0cb3c5c95a63, 0x4ed8aa4ae3418acb, 0x5b9cca4f7763e373,
	0x682e6ff3d6b2b8a3, 0x748f82ee5defb2fc, 0x78a5636f43172f60, 0x84c87814a1f0ab72, 0x8cc702081a6439ec,
	0x90befffa23631e28, 0xa4506cebde82bde9, 0xbef9a3f7b2c67915, 0xc67178f2e372532b, 0xca273eceea26619c,
	0xd186b8c721c0c207, 0xeada7dd6cde0eb1e, 0xf57d4f7fee6ed178, 0x06f067aa72176fba, 0x0a637dc5a2c898a6,
	0x113f9804bef90dae, 0x1b710b35131c471b, 0x28db77f523047d84, 0x32caab7b40c72493, 0x3c9ebe0a15c9bebc,
	0x431d67c49c100d4c, 0x4cc5d4becb3e42b6, 0x597f299cfc657e2a, 0x5fcb6fab3ad6faec, 0x6c44198c4a475817
};


/* DO NOT USE THIS DIRECTLY UNLESS YOU HAVE A GOOD REASON TO.
 * see include/hash.h line 9 for an explanation.
 */
void HCL_sha512_unsafe(unsigned char *msg, uint64_t msg_len, char *out) {
	/* TODOOO : Fix this */
	uint64_t h0 = 0x6a09e667f3bcc908;
	uint64_t h1 = 0xbb67ae8584caa73b;
	uint64_t h2 = 0x3c6ef372fe94f82b;
	uint64_t h3 = 0xa54ff53a5f1d36f1;
	uint64_t h4 = 0x510e527fade682d1;
	uint64_t h5 = 0x9b05688c2b3e6c1f;
	uint64_t h6 = 0x1f83d9abfb41bd6b;
	uint64_t h7 = 0x5be0cd19137e2179;

	/* The padding (unsafe part) */
	uint64_t new_len = msg_len;
	msg[new_len++] = 0x80;

	uint64_t bits = new_len * 8 % 1024;
	while (bits != (1024 - 64)) {
		msg[new_len++] = 0;

		bits = (new_len * 8) % 1024;
	}

	bits = msg_len * 8;
	msg[new_len + 0] = (bits >> 56) & 0xFF;
	msg[new_len + 1] = (bits >> 48) & 0xFF;
	msg[new_len + 2] = (bits >> 40) & 0xFF;
	msg[new_len + 3] = (bits >> 32) & 0xFF;
	msg[new_len + 4] = (bits >> 24) & 0xFF;
	msg[new_len + 5] = (bits >> 16) & 0xFF;
	msg[new_len + 6] = (bits >> 8) & 0xFF;
	msg[new_len + 7] = (bits >> 0) & 0xFF;
	new_len += 8;

	for (uint64_t i = 0; i < new_len; i += 128) {
		uint64_t buf[80];

		/* Load the chunk into the first 16 entries of buf */
		for (int j = 0; j < 16; j++) {
			/* Load the number as big-endian */
			uint64_t num = 0;
			for (int k = 0; k < 8; k++) {
				num <<= 8;
				num |= msg[i + j * 8 + k];
			}
			buf[j] = num;
		}

		/* Extend the first 16 entries into rest of buf */
		for (int j = 16; j < 80; j++) {
			uint64_t s0 = rotr64(buf[j - 15],  1) ^ rotr64(buf[j - 15],  8) ^ (buf[j - 15] >> 7);
			uint64_t s1 = rotr64(buf[j -  2], 19) ^ rotr64(buf[j -  2], 61) ^ (buf[j -  2] >> 6);
			buf[j] = buf[j - 16] + s0 + buf[j - 7] + s1;
		}

		/* Working variables */
		uint64_t A = h0;
		uint64_t B = h1;
		uint64_t C = h2;
		uint64_t D = h3;
		uint64_t E = h4;
		uint64_t F = h5;
		uint64_t G = h6;
		uint64_t H = h7;

		/* --- MAIN LOOP */
		for (int j = 0; j < 80; j++) {
			uint64_t S1 = rotr64(E, 14) ^ rotr64(E, 18) ^ rotr64(E, 41);
			uint64_t ch = (E & F) ^ ((~E) & G);
			uint64_t temp1 = H + S1 + ch + round_constants512[j] + buf[j];

			uint64_t S0 = rotr64(A, 28) ^ rotr64(A, 34) ^ rotr64(A, 39);
			uint64_t maj = (A & B) ^ (A & C) ^ (B & C);
			uint64_t temp2 = S0 + maj;

			H = G;
			G = F;
			F = E;
			E = D + temp1;
			D = C;
			C = B;
			B = A;
			A = temp1 + temp2;
		}

		h0 += A;
		h1 += B;
		h2 += C;
		h3 += D;
		h4 += E;
		h5 += F;
		h6 += G;
		h7 += H;
	}

	/* Produce the output */
	uint64_t res[8] = {
		h0, h1, h2, h3, h4, h5, h6, h7
	};

	for (int i = 0; i < 8; i++) {
		uint64_t val = res[i];
		/* Write the value in big-endian */
		for (int j = 8; j; j--) {
			out[i * 8 + j - 1] = val & 0xFF;
			val >>= 8;
		}
	}
	return;
}


char *HCL_sha512(char *msg, uint64_t msg_len) {
	if (msg == NULL) return NULL;

	/* Ensure there's enough space for HCL_sha1_unsafe to pad the message */
	uint64_t mem_len = msg_len + (128 - (msg_len % 128));
	unsigned char *mem = malloc(mem_len);
	if (mem == NULL) return NULL;

	memset(mem, 0, mem_len);
	memcpy(mem, msg, msg_len);

	/* Create the output buffer */
	char *out = malloc(64);
	if (out == NULL) {
		free(mem);
		return NULL;
	}

	HCL_sha512_unsafe(mem, msg_len, out);

	free(mem);
	return out;
}
